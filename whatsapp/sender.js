...
/**
 * Chave única: numero_categoria_YYYYMMDD
 * → mesmo número pode receber campanhas diferentes em dias diferentes.
 * → nunca recebe a mesma campanha duas vezes no mesmo dia.
 */
function logKey(numero, categoria) {
  const today = new Date().toISOString().slice(0, 10);
  return `${String(numero).replace(/\D/g, '')}_${categoria}_${today}`;
}
...
/** Detecta o caminho do Chrome instalado no sistema operacional */
function findChrome() {
  const candidates = [
    // Windows
  ...
  return null; // puppeteer tentará usar o bundled
}
...
async function main() {
  ...
  const results = { sent: [], failed: [], skipped: [], notRegistered: [] };
  ...
  for (let i = 0; i < limited.length; i++) {
    ...
    // Enviar com retry
    const result = await sendWithRetry(client, phone, contact.mensagem);
    ...
    // Delay com jitter (exceto no último)
